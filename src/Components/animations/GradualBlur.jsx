...
const useScrollFade = (enabled, threshold, baseOpacity) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (distFromBottom <= threshold) {
        setOpacity(baseOpacity * (distFromBottom / threshold));
      } else {
...
const GradualBlur = props => {
  ...
  const isPageBottom = config.target === 'page' && config.position === 'bottom';
  const scrollOpacity = useScrollFade(isPageBottom, config.threshold, config.opacity);
  ...
  const containerStyle = useMemo(() => {
    ...
    const baseStyle = {
      ...
      opacity: isPageBottom ? scrollOpacity : (isVisible ? 1 : 0),
    ...
    return baseStyle;
  }, [config, responsiveHeight, responsiveWidth, isVisible, scrollOpacity, isPageBottom]);
